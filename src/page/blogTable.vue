...
<template>
  <div>
    ...
    <!--<div style="width: 100%height: 1pxbackground-color: #20a0ffmargin-top: 8pxmargin-bottom: 0px"></div>-->
    <el-table
      ref="multipleTable"
      :data="articles"
      tooltip-effect="dark"
...
export default{
  ...
  mounted: function () {
    ...
    _this.$on('blogTableReload', function () {
      _this.loading = true
  ...
  methods: {
    ...
    loadBlogs (page, count) {
      ...
      _this.$api.post(url, param, resp => {
        _this.loading = false
        if (resp.code === 0) {
          _this.articles = resp.data.list[0].voList
          _this.totalCount = resp.data.list[0].total
        } else {
    ...
    deleteToDustBin (state) {
      ...
      this.$confirm(state !== 2 ? '将该文件放入回收站，是否继续?' : '永久删除该文件, 是否继续?', '提示', {
        ...
        let param = {
          ids: JSON.stringify(_this.dustbinData),
          state: state
        }
        _this.$api.post(url, param, resp => {
          if (resp != null) {
            var code = resp.code
            _this.$message({type: code, message: resp.message})
            if (code === 0) {
              _this.$emit('blogTableReload')// 通过选项卡都重新加载数据
